...
function Results() {
  ...
  return (
    <>
        ...
        <div className='w-[100%] flex flex-col justify-start items-center relative mb-4'>
            ...
            <p className="mt-[11px] text-[17px] font-syne text-white font-semibold ">Результаты</p>
            <div dangerouslySetInnerHTML={{ __html:  text.first_text.replace(/\n/g, "<br/>")}} className='w-[90%] text-white font-sans'></div>
            {
  text.exercises &&
  Object.entries(text.exercises).map(([key, value]) => {
    ...
    return (
      <div key={key} className="w-[100%] flex flex-col justify-start items-center">
        <p className="text-white mt-4 font-semibold font-sans w-[90%]">{exerciseName}</p>
        {gifUrl && (
          <img src={gifUrl} alt={gifAlt} className="w-[90%] rounded-[10px] mt-2" />
        )}
      </div>
    );
  })
}
<div dangerouslySetInnerHTML={{ __html:  text.last_text.replace(/\n/g, "<br/>")}} className='w-[90%] mt-4 text-white font-sans'></div>
